...
class PersonDetector():
    ...
    def CamRgbImageCallback(self, rgb_image_data):
        ...
        if self.person_bbox.probability > 0.0 :
           ...
           # 一旦、BoundingBoxの中心位置の深度を取得 (今後改善予定）
            ...
            m_person_depth2 = self.m_depth_image[(int)(self.box.ymax+self.box.ymin)/2][(int)(self.box.xmax+self.box.xmin)/2]
            ...
            cv2.rectangle(rgb_image, (self.box.xmin, self.box.ymin), (self.box.xmax, self.box.ymax),(0,0,255), 2)

            #rospy.loginfo('Class : person, Score: %.2f, Dist: %dmm ' %(self.person_bbox.probability, m_person_depth))
            rospy.loginfo('Class : %s, Score: %.2f, Dist: %.2fm ' %(self.box, self.person_bbox.probability*100, m_person_depth))
            rospy.loginfo('Class : %s, Score: %.2f, Dist: %.2fm ' %(self.box, self.box.probability*100, m_person_depth2))
            #text = "person " +('%dmm' % m_person_depth)

            text = (" person " % self.person_bbox) +('%.2fm' % m_person_depth)
            text = (" Class : %s " % self.box) +('%.2fm' % m_person_depth2)

            self._msg.header.stamp = rospy.Time.now()        #msg
            self._msg.position = [m_person_depth,m_person_depth2,1]
            ...
            text_top2 = (self.box.xmin, self.box.ymin - 10)
            text_bot2 = (self.box.xmin + 80, self.box.ymin + 5)
            text_pos2 = (self.box.xmin + 5, self.box.ymin)
            cv2.rectangle(rgb_image, text_top2, text_bot2, (0,0,0),-1)
            cv2.putText(rgb_image, text2, text_pos2, cv2.FONT_HERSHEY_SIMPLEX, 0.35, (255, 0, 255), 1)
    ...
    def DarknetBboxCallback(self, darknet_bboxs):
        ...
        for self.box in darknet_bboxs.bounding_boxes:
            a=1
